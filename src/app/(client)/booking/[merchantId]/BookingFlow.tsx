...
export default function BookingFlow({ merchantId, initialMerchant }: BookingFlowProps) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const preselectedServiceId = searchParams.get("service");

  // Pre-select service if passed via URL query param
  const preselectedService =
    preselectedServiceId && initialMerchant.services
      ? initialMerchant.services.find((s) => s.id === preselectedServiceId) ?? null
      : null;
